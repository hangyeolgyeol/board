...
/*
    ...
    @Index: 데이터베이스 인덱스는 추가 쓰기 같은 저장공간을 미리 잡아놓는 애니테이션
...
public class Ex01_1_Article_엔티티로_등록 {
    ...
    @Id /* 기본 키(PK) 와 객체의 필드를 매핑시켜주는 애니테이션
            Id만 사용할경우: 키 직겁 할당 해야함
    ...
    /* @Setter도 @Getter 처럼 클래스 단위로 걸 수 있는데 */
    private Long id; // 고유번호


    @Setter
    @Column(nullable = false) // 해당 컬럼이 not null인 경우 @Column(nullable = false) 써줘야함(기본값은 true)
    private String title; // 제목  // NOT NULL, varchar(255)

    @Setter
    @Column(nullable = false, length = 10000) // not null varchar(10000)
    private String content; // 본문  // NOT NULL, varchar(10000)

    @Setter
    private String hashtag; // 해시태그  // Null, varchar(255)



    // 양방향 바인딩 (Ex01_2 의 // 연관고나계 매핑. 다 끝나고 와서 하는거)
    ...
    /* 엄청 중요!!
        ...
        @ToString이 모든 필드들을 다 찍고 Set<Ex01_2_AriticleComment_엔티티로_등록>꺼를 찍으려고 하는데
        그러면 ArticleComment파일 가서도 거기있는 @ToString이 모든 원소들을 다 찍으려고 하면서 Ex01_1_Article_엔티티로 등록
        이라는걸 보는 순간 다시 Article의 @ToString이 동작하면서 또 모든 원소들을 찍을거임. 이런식으로 서로가 서로를 호출하면서
        순환 참조를 하게 되면서 메모리가 터져서 시스템이 다운될 수 있다.
    ...
    */
    ...
    /* 이렇게 어노테이션만 붙여주기만 해도 auditing이 작동한다.
        @CreatedDate: 최초에 insert 할때 자동으로 넣어준다.
        @CreatedBy: 최초에 insert 할때 자동으로 한번 넣어준다.
        @LastModifiedDate: 작성 당시의 시간을 매번 실시간으로 넣어준다.
        @LastModifiedBy: 작성 당시의 이름을 매번 실시간으로 넣어준다.

        위 4개중에 생성일시나 수정일시는 알아낼 수 있는데, 최초의 생성자는 테스트시 로그인을 하고 온게 아니기 때문에 따로 알아낼수가 없다.
        그래서 config > JpaConfig 의 public AuditorAware<String> auditorAwre() {} 를 사용한다
        리턴타입이 String이기 때문에 createdBy 나 modifiedBy 에 들어가질 수 있다.
    */

    protected Ex01_1_Article_엔티티로_등록() {}

    /* 사용자가 입력하는 값만 받는 생성자 생성. 나머지 메타들은 시스템이 알아서 하게 해줘야 함 */
...
/*
    ...
    heshcode - 객체를 식별하는 Integer 값
               ...
               heshcode 사용하는 이유는 객체를 비교할 때 드는 비용이 낮다
