...
@AutoConfigureMockMvc /* 테스트를 돌리면 콘솔창에 Hibernate 부분에 select 쿼리가 같이 나온다.
                        이 말은 실제 repository 까지 실행시켰다는 뜻 한마디로 DB를 건드렸을수도 있다는 뜻이다.
                        그래서 이건 테스트니까 트랜잭션으로 묶어서 롤백까지 시켜주기 위해서 사용 */
@Transactional
public class Ex04_2_DataRestRepositoryTest {

    /* 빈 준비
        MockMvc: 실제 객체와 비슷하지만 테스트에 필요한 기능만 가지만 가지고 있는 가짜 객체를 만들어서 실제 서버를 돌리지 않고도 스프링
                 MVC 동작을 할 수 있는 클래스
    */
