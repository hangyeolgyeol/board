...
/* 컨트롤러에 대한 테스트 (슬라이스 테스트)
    ...
    - 대표적인 통합 테스트 어노테이션
        1) @SpringBootTest - 스프링이 관리하는 모든 빈을 등록 시켜서 테스트 함( BoardApplicationTests.java 에 있음 )

    - 대표적인 슬라이스 테스트 어노테이션
        1) @WebMvcTest - Contoller를 테스트 할때 주로 사용
...
@WebMvcTest // 슬라이스 테스트. 컨트롤러 관련된 Bean들만 로드 함.
            // 이 어노테이션은 컨트롤러와 연관된 내용만 최소한으로 로드 하기 때문에 data rest 의 autoConfigration을 읽지는 않는다.
public class Ex04_1_DataRestRepositoryTest {
    /* MockMvc 테스트 방법
        ...
        Hal 에서 http://localhost:8080/api/article 를 돌려보면 동장하는데 왜 여기서는 안되냐면
        @WebMvcTest 는 슬라이스 테스트 이기 때문에 controller 이외의 빈은 아예 로그하지 않기 때문임
        그래서 이런 경우에는 @WebMvcTest 대신 통합테스트를 사용할거임.
    */
    ...
    void test1() throws Exception {
        ...
        /* get() 작성하는 법
            ...
            2) 추천에 getClass()만 뜰건데 그 상태에서 Ctrl + space 누른다
            3) 추천 리스트가 뜨는데 그중에 MockMvcRequestBuilders 있을거임. 그거 alt + enter 선택
                윈도우: alt + enter
                맥: 옵션 + 엔터
